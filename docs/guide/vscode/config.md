...
```json
"editor.fontFamily": "'CaskaydiaCove Nerd Font Mono', 'CaskaydiaCove Nerd Font Propo', 'CaskaydiaCove Nerd Font'",
"editor.fontLigatures": true, // 是否开启连体字
```
...
## VSCode 多个窗口显示多个选项卡（仅支持macOS）

> vscode > settings > `window.nativeTabs` true
